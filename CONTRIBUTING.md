...
This project uses API keys for AI model integration. Follow these security steps:
...
⚠️ **Security Note**: Never commit `.env` files or API keys to the repository. The project includes GitHub secret scanning and push protection to prevent accidental exposure.
...
When contributing to this project, follow these security best practices:
...
#### **Git Security**
- 🔐 **Check before committing**: `git status` should not show `.env` files
- 🔐 **Use environment variables** for CI/CD pipelines
- 🔐 **Rotate keys** if accidentally exposed
- 🔐 **Monitor usage** through API provider dashboards

#### **Development Security**
```bash
# Verify security setup
grep .env .gitignore  # Should show .env patterns
...
1. **Immediate Action**: Remove the file from git history
   ```bash
...
2. **Regenerate Keys**: Contact the API provider to rotate exposed keys

3. **Force Push**: Update the repository (use with caution)
   ```bash
   git push origin --force
   ```

4. **Verify**: Ensure the secret is no longer in the repository history
